...
public:
  ...
  : Node("landmarks")
  {
    ...
    publish_landmarks = create_publisher<visualization_msgs::msg::MarkerArray>(
      "landmarks",
      qos_profile);
    publish_fittings =
      create_publisher<visualization_msgs::msg::MarkerArray>("fitting", qos_profile);
  }
      ...
      clusters.erase(
        remove_if(
          ...
          [](const std::vector<turtlelib::Point2D> & cluster) {
            return cluster.size() < 10 || cluster.size() > 30;
          }),
        clusters.end());
    } else {
      clusters.erase(
        remove_if(
          clusters.begin(), clusters.end(),
          [](const std::vector<turtlelib::Point2D> & cluster) {
            return cluster.size() < 4 || cluster.size() > 20;
          }),
        clusters.end());
    }
