...
namespace turtlelib
{
    ...
    void Svg::draw_point(std::ofstream &svgFile, Point2D &p, const std::string &stroke_color, const std::string fill_color){
        svgFile<<"<circle cx=\""<<p.x<<"\" cy=\""<<p.y<<"\" r=\"3\" stroke=\""<<stroke_color<<"\" fill=\""<<fill_color<<"\" stroke-width=\"1\" marker-start=\"url(#Arrow1Sstart)\"  />"<<std::endl;
    }

    void Svg::draw_vector(std::ofstream &svgFile, Point2D &tail, Point2D &head, const std::string &stroke_color){
